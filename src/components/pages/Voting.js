...
import { db, doc, getDoc, query, where, collection, getDocs } from "../../firebase/config";
...
const Voting = () => {
  ...
  const fetchCandidateData = async () => {
    const q = query(collection(db, "candidate-details"), where("wardnum", "==", "55"));
  ...
  candidateDetails.forEach((candidate) => {
    console.log(candidate.data().firstname, candidate.data().lastname, " - ", candidate.data().partyname)
  });
    ...
    const votepaperstyle={padding: 30 , height:'60vh', width:'120vh',margin:"40px auto"}
    ...
    return (
      <Grid container /*spacing={2}*/ style={{backgroundColor: 'rgb(255, 194, 0, 25%)', height: '89vh'}}>  
      {/* Displaying the voter details */}
            ...
            <List>
              <ListItem alignItems="flex-start">
                <ListItemAvatar>
                  <Avatar alt="partyname" src="/static/images/avatar/1.jpg" />
                </ListItemAvatar>
                <ListItemText
                  primary="BJP"
                  secondary={
